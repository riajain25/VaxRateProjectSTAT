...
plot_glmnet(ridge_fit, data_train, features_to_plot = 5)
...
plot_glmnet(lasso_fit, data_train, features_to_plot = 5)
...
lasso_features %>% filter(coefficient != 0) %>% arrange(desc(abs(coefficient)))


#Elastic Net

elnet_fit = cva.glmnet(Series_Complete_18PlusPop_Pct ~ ., 
                       nfolds = 10, 
                       data = data_train)
...
plot_glmnet(elnet_fit_best, data_train)

plot_glmnet(elnet_fit_best, data_train, features_to_plot = 5)

elnet_features = extract_std_coefs(elnet_fit_best, data_train)
elnet_features
elnet_features %>% filter(coefficient != 0) %>% arrange(desc(abs(coefficient)))
