...
elnet_RMSE = sqrt(mean(elnet_predictions - data_test$Series_Complete_18PlusPop_Pct)^2)
...
data_train %>% summarise(meanvaxrate = mean(Series_Complete_18PlusPop_Pct))

intercept_onlyRMSE = sqrt(mean(.565 - data_test$Series_Complete_18PlusPop_Pct)^2)

# print nice table
tibble(Method = c("Intercept-Only", "OLS Regression", "Ridge", "Lasso", "Elastic Net" ,"Regression Tree", "Random Forest", "Boosting"), `Test RMSE` = c(intercept_onlyRMSE,lm_RMSE, ridge_RMSE, lasso_RMSE, elnet_RMSE, opregtree_RMSE, rf_RMSE, gbm_RMSE))
